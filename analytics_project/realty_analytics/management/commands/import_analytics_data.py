...
from realty_analytics.models import AnalyticsDataPoint
from decimal import Decimal, InvalidOperation
from datetime import datetime
from routines import get_isodate_from_string

class Command(BaseCommand):
    help = 'Массовый импорт или обновление данных для AnalyticsDataPoint из текстового файла ";"'

    def add_arguments(self, parser):
        parser.add_argument('file_path', type=str, help='Путь к текстовому файлу для импорта')
        # Добавим опциональный флаг для полной очистки таблицы перед импортом
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Очистить таблицу AnalyticsDataPoint перед импортом',
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if kwargs['clear']:
            self.stdout.write(self.style.WARNING("Очистка таблицы AnalyticsDataPoint..."))
            AnalyticsDataPoint.objects.all().delete()
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for i, row in enumerate(reader, 1):
                    ...
                    date_str, city_id_str, prop_type, prop_class, price_str = row
                    ...
                    unique_key = (date_obj, city_id, prop_type, prop_class)
                ...
                if objects_to_create:
                    AnalyticsDataPoint.objects.bulk_create(objects_to_create, ignore_conflicts=True)
